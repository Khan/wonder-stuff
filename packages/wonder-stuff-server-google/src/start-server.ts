 ...
 */
// @ts-expect-error - TS2315 - Type 'Application' is not generic.
export async function startServer<TReq extends RequestWithLog<Request>, TRes extends Response>(options: ServerOptions, app: Application<TReq, TRes>): Promise<http.Server | null | undefined> {
...
// @ts-expect-error - TS2322 - Type 'unknown' is not assignable to type '"production" | "test" | "development"'.
...
// @ts-expect-error - TS2322 - Type 'string | number | symbol' is not assignable to type 'string | undefined'.
                      level: logLevel,
                  })
                : null,
    });
...
// @ts-expect-error - TS2345 - Argument of type 'unknown' is not assignable to parameter of type '"production" | "test" | "development"'.
    await setupIntegrations(restOptions.mode, integrations);

    // Add GAE middleware to the app.
// @ts-expect-error - TS2345 - Argument of type 'unknown' is not assignable to parameter of type '"production" | "test" | "development"'.
    const appWithMiddleware = await addAppEngineMiddleware<TReq, TRes>(app, restOptions.mode, logger);

    // Start and return the server.
    return startServerCore(
// @ts-expect-error - TS2345 - Argument of type '{ logger: any; includeRequestMiddleware: boolean; name: unknown; port: unknown; host: unknown; mode: unknown; keepAliveTimeout?: unknown; allowHeapDumps?: unknown; }' is not assignable to parameter of type 'ServerOptions'.
        {
            ...restOptions,
