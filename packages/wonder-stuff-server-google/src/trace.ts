...
interface ITrace {
     ...
     */
    <TReq extends RequestWithLog<Request>>(action: string, message: string, request?: TReq): ITraceSession;
    /**
     * Start tracing an event.
     *
     * This will log the start of a trace and open a trace session, which is
 ...
 */
export const trace: ITrace = (action: string, message: string, requestOrLogger: unknown): ITraceSession => {
...
// @ts-expect-error - TS2345 - Argument of type 'unknown' is not assignable to parameter of type 'RequestWithLog<Request<ParamsDictionary, any, any, ParsedQs, Record<string, any>>> | undefined'.
