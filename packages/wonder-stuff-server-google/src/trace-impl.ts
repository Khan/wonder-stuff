// @ts-expect-error - TS2305 - Module '"@google-cloud/trace-agent"' has no exported member 'Tracer'.
 ...
 */
export const traceImpl = (logger: Logger, action: string, message: string, tracer?: Tracer): ITraceSession => {
    if (!action) {
        throw new KindError(
    ...
    const end = (info?: TraceSessionInfo): void => {
         ...
         */
// @ts-expect-error - TS2345 - Argument of type 'MemoryUsage' is not assignable to parameter of type 'NumberObj'.
