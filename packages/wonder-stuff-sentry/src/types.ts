...
// we need to use.
type SentrySeverity = 'fatal' | 'error' | 'warning' | 'log' | 'info' | 'debug' | 'critical';
...
export type SentryEventProcessor = (event: SentryEvent, hint?: SentryEventHint) => Promise<SentryEvent | null | undefined> | SentryEvent | null | undefined;

export interface SentryHub {
 getIntegration<T extends SentryIntegration>(integration: Flow.Class<T>): T | null | undefined;
}
