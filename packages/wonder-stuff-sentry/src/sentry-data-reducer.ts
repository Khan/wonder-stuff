import {safeStringify, getOriginalStackFromError} from "@khanacademy/wonder-stuff-core";
...
export const sentryDataReducer = (
    ...
    const tags = {...accumulatorTags, ...currentTags} as const;

    // Combine contexts by spreading the current contexts over the accumulator
    // contexts.
    const contexts = {...accumulatorContexts, ...currentContexts} as const;
    ...
    if (index !== 0) {
// @ts-expect-error - TS2542 - Index signature in type '{ readonly [x: string]: Metadata; }' only permits reading.
        contexts[
