 ...
 */
export const extractHashFromFileName = (fileName: string): string => (HASH_FILENAME_RE.exec(fileName) || [])[1];
 ...
 */
export const hashFileContents = (contents: string): string => crypto.createHash("md5").update(contents).digest("hex").slice(0, 20);
